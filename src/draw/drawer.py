...
class Drawer:
    ...
    def save_great_square_solutions(self, pattern, solutions):
        ...
        for index, solution in enumerate(solutions):
            ...
            self.__draw_card(
                solution["center_middle"],
                3 * self.CELL_SIZE,
                3 * self.CELL_SIZE,
                draw,
            )
            self.__draw_card(
                solution["center_right"],
                6 * self.CELL_SIZE,
                3 * self.CELL_SIZE,
                draw,
            )
    ...
    def __draw_card(self, card, x, y, drawer):
        drawer.rectangle(
            ...
            fill=self.__get_color(card.top_left),
            outline="black",
        )
        drawer.rectangle(
            (x + self.CELL_SIZE, y, x + 2 * self.CELL_SIZE, y + self.CELL_SIZE),
            fill=self.__get_color(card.top_right),
            outline="black",
        )
        drawer.rectangle(
            (
            ...
            fill=self.__get_color(card.bottom_right),
            outline="black",
        )
        drawer.rectangle(
            (x, y + self.CELL_SIZE, x + self.CELL_SIZE, y + 2 * self.CELL_SIZE),
            fill=self.__get_color(card.bottom_left),
            outline="black",
        )
